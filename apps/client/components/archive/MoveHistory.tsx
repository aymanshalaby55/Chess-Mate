...
const MoveHistory: React.FC<MoveHistoryProps> = ({ 
  ...
  const groupedMoves: Array<{ white: string; black: string | null; whiteSan: string; blackSan: string | null }> = [];
  ...
  for (let i = 0; i < history.length; i += 2) {
    ...
    groupedMoves.push({
      ...
      black: blackMove ? `${blackMove.from}-${blackMove.to}${blackMove.promotion ? `=${blackMove.promotion.toUpperCase()}` : ''}` : null,
  ...
  return (
    <div className="border rounded-md p-4 bg-white shadow-sm h-full overflow-auto">
      <h3 className="text-lg font-semibold mb-4 text-center border-b pb-2">Move History</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50">
                <th className="py-2 px-2 text-left font-medium text-gray-700 w-10">#</th>
                <th className="py-2 px-2 text-left font-medium text-gray-700">White</th>
                <th className="py-2 px-2 text-left font-medium text-gray-700">Black</th>
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={index} className={index % 2 === 1 ? 'bg-gray-50' : ''}>
                  ...
                  <td 
                    className={`py-2 px-2 font-mono hover:bg-blue-100 cursor-pointer rounded transition-colors duration-150 ${
                      currentMoveIndex === index * 2 ? 'bg-blue-200 font-bold' : ''
                    }`}
                  ...
                  <td 
                    className={`py-2 px-2 font-mono ${
                      move.black ? 'hover:bg-blue-100 cursor-pointer' : 'text-gray-400'
                    } rounded transition-colors duration-150 ${
                      currentMoveIndex === (index * 2) + 1 ? 'bg-blue-200 font-bold' : ''
                    }`}
