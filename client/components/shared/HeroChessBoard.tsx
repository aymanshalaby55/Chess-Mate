...
export default function RealisticChessboard() {
  ...
  useEffect(() => {
    ...
    const drawChessboard = () => {
      ...
      for (let row = 0; row < 8; row++) {
        for (let col = 0; col < 8; col++) {
          ...
          if (row === 7) {
            ctx.fillText(String.fromCharCode(97 + col), (col + 1) * squareSize - 3, (row + 1) * squareSize - 3)
          }
          if (col === 0) {
            ctx.textAlign = "left"
            ctx.textBaseline = "top"
            ctx.fillText(String(8 - row), col * squareSize + 3, row * squareSize + 3)
          }
        }
      }
    ...
    const drawPiece = (piece: PieceType, col: number, row: number, squareSize: number) => {
      if (!ctx || !piece) return
      const symbol = pieceSymbols[piece]
      const centerX = col * squareSize + squareSize / 2
      const centerY = row * squareSize + squareSize / 2 + squareSize * 0.1
